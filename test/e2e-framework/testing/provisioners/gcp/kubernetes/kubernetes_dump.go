...
func getClusterOutputWithSsh(out *strings.Builder, sshClient *ssh.Client) (err error) {
	...
	allOutput, err := infra.SshRunCommand(sshClient, getAllCommand)
	if err != nil {
	...
	nonRuningPodsList, err := infra.SshRunCommand(sshClient, getAllNonRuningPods)
	if err != nil {
	...
	podLogsTemplate := "%s oc logs --all-container --prefix %s %s"
	for _, line := range strings.Split(string(nonRuningPodsList), ",") {
		...
		command := fmt.Sprintf(podDescribeTemplate, prefix, parts[0], parts[1])
		podDescribe, err := infra.SshRunCommand(sshClient, command)
		if err != nil {
		...
		fmt.Fprintf(out, "---------- Failed pod (%s/%s) ----------\n%s\n", parts[0], parts[1], string(podDescribe))

		command = fmt.Sprintf(podLogsTemplate, prefix, parts[0], parts[1])
		podLogs, err := infra.SshRunCommand(sshClient, command)
		if err != nil {
...
func DumpOpenshiftClusterState(ctx context.Context, name string) (ret string, err error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to marshall kubeconfig to Yaml format: %w", err)
		errs = append(errs, err)
		return
	}

	fmt.Fprintf(&out, "Patched kubeconfiguration:\n%s\n", kubeconfigYaml)
