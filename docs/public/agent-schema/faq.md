...
1. **Determine the node type.** If the setting holds a value (string, number,
   boolean, list, or dict that *is* the value), it is a leaf node. If it groups
   other settings, it is a section node.

2. **Choose the correct schema file.**
   - Settings for `datadog.yaml` go in `pkg/config/schema/core_schema_enriched.yaml`.
   - Settings for `system-probe.yaml` go in
     `pkg/config/schema/system-probe_schema_enriched.yaml`.
...
4. **Add the node** with the mandatory keywords:
   - Leaf nodes require `type` and either `default` or `platform_default`.
...
5. **Add optional keywords** as appropriate:
   ...
   - `visibility: public` — add when the setting should appear in
     `datadog.yaml.example` and public documentation (see
...
/// note
[WIP] Automated solution coming soon.
///
...
1. **Write a `description`** that explains what the setting does, what the
   default means, and any caveats. Every public setting must have a description.

2. **Add `visibility: public`** to the leaf or section node.

3. **Make every parent section public too.** A leaf node with
...
The setting will appear in `datadog.yaml.example` the next time the file is
regenerated.
...
/// note
This section will be updated once the generator is fully implemented. The invoke
task will be documented here so that developers can run it locally during
iteration.
///
...
- **`datadog.yaml.example`** is generated from
  `pkg/config/schema/core_schema_enriched.yaml` and covers the main Agent configuration —
  metrics, logs, APM, processes, and more.

- **`system-probe.yaml.example`** is generated from
  `pkg/config/schema/system-probe_schema_enriched.yaml` and covers only the settings
