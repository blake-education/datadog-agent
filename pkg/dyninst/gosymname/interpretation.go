...
type Interpretation struct {
	// Package is the unescaped import path of the outer function.
	Package string
	...
	ABISuffix string
	// Confidence is a heuristic score from 0.0 to 1.0.
	Confidence float32
}
...
// QualifiedName reconstructs the qualified name, e.g. "pkg.(*T).Method".
func (i *Interpretation) QualifiedName() string {
	var b []byte
	if i.Package != "" {
		b = append(b, i.Package...)
		b = append(b, '.')
	}
...
type InlinedCall struct {
	// Package is set for cross-package inlined calls.
	Package string
	// Receiver is the receiver type name, if this is a method call.
	Receiver string
	// ReceiverKind is ReceiverPointer, ReceiverValue, or ReceiverNone.
...
func (ic *InlinedCall) IsMethod() bool {
	return ic.ReceiverKind != ReceiverNone
}

// QualifiedFunction returns the qualified function name, e.g.
// "pkg.(*T).Method" or just "Function" for simple cases.
func (ic *InlinedCall) QualifiedFunction() string {
	var b []byte
	if ic.Package != "" {
		b = append(b, ic.Package...)
		b = append(b, '.')
	}
	switch ic.ReceiverKind {
	case ReceiverPointer:
		b = append(b, "(*"...)
