...
// HasCrossPackageInlining returns true if the local part contains '/',
// indicating cross-package inlined function names.
func (s *Symbol) HasCrossPackageInlining() bool {
	return strings.ContainsRune(s.local, '/')
}
...
// Best returns the interpretation with the highest confidence score, or nil
// if there are no interpretations.
func (s *Symbol) Best() *Interpretation {
	s.ensureInterps()
	if len(s.interps) == 0 {
		return nil
	}
	best := &s.interps[0]
	for i := 1; i < len(s.interps); i++ {
		if s.interps[i].Confidence > best.Confidence {
			best = &s.interps[i]
		}
	}
	return best
...
// Disambiguate returns only the interpretations for which keep returns true.
func (s *Symbol) Disambiguate(keep func(*Interpretation) bool) []Interpretation {
	s.ensureInterps()
	var result []Interpretation
	for i := range s.interps {
		if keep(&s.interps[i]) {
			result = append(result, s.interps[i])
		}
	}
	return result
}
